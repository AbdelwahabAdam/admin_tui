...
#--------------------------------------------------------------------------------------#
class Side_NavBar():
    def __init__(self,  entries, select=0, myparent=None, entries_color='#00ff44'):
            ...
            content=HSplit([
                Window(
                    ...
                    height=Dimension(preferred=len(
                        self.navbar_entries), max=len(self.navbar_entries)+1),
                    cursorline=True,
...
#--------------------------------------------------------------------------------------#
class Custom_Dialog():
    def __init__(self, title=None, button_functions=[], entries_list=[], entries_color='#00ff44'):
    ...
    def create_window(self):
        self.dialog = Dialog(
            ...
            body=HSplit(
                [
                    VSplit([Label(text=str(self.entries_list[i][1]), dont_extend_height=True),
...
#--------------------------------------------------------------------------------------#
#--------------------------------------------------------------------------------------#
class ListBox_Horizontal():

    def __init__(self, myparent, headers, selectes, data=None, headerColor='green', entriesColor='white'):
        self.myparent = myparent            # ListBox parent class
        self.headers = headers              # ListBox headers
        self.dialog_visible = False         # popup dialog flag
...
#--------------------------------------------------------------------------------------#
    ...
    def create_window(self):
        ...
        self.dialog = Custom_Dialog(
            title="Add New user",
            entries_list=[
                (self.ClientID, "Please type Client ID:"),
                (self.ClientName, "Please type Client Name:"),
                (self.GrantType, "Please type Grant Type:"),
                (self.AccessToken, "Please type Access Token:"),
                (self.Helper, ''),
            ], button_functions=[
                (self.save_dialog, "OK"),
                (self.hide_dialog, "Cancel")
            ]
        )
    ...
    def handel_Data_spaces(self):
        for i in range(len(self.data)):
            for k in range(len(self.spaces)):
                if len(self.data[i][k]) != self.spaces[k]:
                    # print( self.spaces[k])
                    # print(self.data[i][k])
                    # print((self.spaces[k] -len(self.data[i][k]) ))
                    self.data[i][k] = (
                        self.data[i][k] + " " * (self.spaces[k] - len(self.data[i][k])))
                else:
                    pass
                    # print(self.data[i][k])
#--------------------------------------------------------------------------------------#
...
class JansCliApp(Application):
    ...
    def prapare_containers(self):
        ...
        self.clients = ListBox_Horizontal(
            myparent=self,
            ...
            data=[
                ...
                ['c64bc7a2-ff35-4166-8174-9436f74d4c38', 'Ecommerce Support Site',
        ...
        self.side_NavBar = Side_NavBar(myparent=self, entries=['Person Authentication',
                                                               'Consent Gathering',
                                                               'Post Authentication',
                                                               'id_token',
                                                               'Password Grant',
