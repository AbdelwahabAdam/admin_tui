...
data = {
    "displayName": _("Test Client 2"),
    "clientSecret": _("TopSecret 2"),
    "redirectUris": [_("https://www.jans.ion1/cb")],
    "responseTypes": [ _("id_token")],
    "applicationType": _("native")
}


multiLineText = ['redirectUris']
checkBoxesList = ['responseTypes']
radioButtonsList = ['applicationType']


def format_data(form_data) :
    data = {}
    
    for key in form_data.keys() :
        if key in multiLineText:
            data[key]="\n".join(form_data[key])

        elif key in checkBoxesList:
            if len(form_data[key]) >0 :
                for items in form_data[key]:
                    data['{}:{}'.format(key,items)]= '1'
        else :
...
class Custom_CheckBox (CheckBox):
    def __init__(self, text,label=None, name=None, on_change=None, **kwargs):
        super().__init__(text, label, name, on_change, **kwargs)
...
class CLIFrame(Frame):
    ...
    def checkBoxButtons(self, label, name, items):
        for i, o in enumerate(items):
            ...
            self.layout.add_widget(Custom_CheckBox(label=labels, text=o, name="{}:{}".format(name, o), on_focus=self.update_help))
